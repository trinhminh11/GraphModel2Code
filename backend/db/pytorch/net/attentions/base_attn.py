...
class {identifier}(nn.Module):
    ...
    def shape_check(self, Q: Tensor, K: Tensor, V: Tensor):
    ...
    def _forward(
        self, Q: Tensor, K: Tensor, V: Tensor, attn_mask: Optional[Tensor], *args, **kwargs
    ):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if H != K.shape[1] or H != V.shape[1]:
            raise ValueError(f"Q, K and H must have same number of heads after expansion, got Hq={{H}}, Hk={{K.shape[1]}}, Hv={{V.shape[1]}}")
        ...
        if out.shape != Q.shape:
            raise ValueError(f"Output shape {{out.shape}} does not match Q shape {{Q.shape}}")

        return self.post_forward(out), weights
''',
    "sdp_attn": '''
class {identifier}({attn_base}):
    """
    {description}
    """
    ...
    def __init__(self, dropout: float = 0.0):
        """
        Args:
            dropout: Dropout probability applied to the attention weights. Default is 0.0.
        """
        super().__init__()
        self.dropout = dropout

    def _forward(
        self, Q: Tensor, K: Tensor, V: Tensor, causal: bool = False, attn_mask: Optional[Tensor] = None
    ):
        # Q: (B, H, N, D)
        # K: (B, H, Nkv, D)
        # V: (B, H, Nkv, Dv)
...
register_module(
    ModuleNode(
        display_name="Attention Base",
        name="attn_base",
        class_name="AttentionBase",
        description="""An abstract base class for attention head mechanisms.
...
""",
        ...
        forward_kwargs={
            "Q": ("Tensor", __REQUIRED__, "The query tensor"),
            "K": ("Tensor", __REQUIRED__, "The key tensor"),
            "V": ("Tensor", __REQUIRED__, "The value tensor"),
            "causal": ("bool", False, "Whether to apply causal masking"),
            "attn_mask": ("Optional[Tensor]", None, "The attention mask"),
            # "args": ("*", None, "Variable length argument list"),             # TODO: add *arg support
            # "kwargs": ("**", None, "Variable length keyword argument list"),  # TODO: add **kwarg support
        },
        outputs=(("Tensor", "The output tensor"), ("Tensor", "The attention weights")),
        is_abstract=True,
        code_file=("attentions", "base_attn"),
        tags = {Tags.ATTENTION, Tags.ABSTRACT},
    )
)

register_module(
    ModuleNode(
        display_name="Scaled Dot Product Attention (Attention)",
        name="sdp_attn",
        class_name="ScaledDotProductAttention",
        description="""The original Attention
    ...
    Methods:
        _forward(Q, K, V, causal=False):
            Computes the scaled dot-product attention.
                ...
                Q (Tensor): Query tensor of shape (B, H, N, D).
                K (Tensor): Key tensor of shape (B, H, N, D).
                V (Tensor): Value tensor of shape (B, H, N, Dv).
    ...
    Raises:
        ...
        node_dependencies={
            "attn_base": get_module("attn_base"),
        },
        kwargs={
